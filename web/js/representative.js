...
!(function(exports, doc, $, _, setImmediate, setTimeout, presentAlert, validateForm, OT,
  ...
  var representativeName, sessionId;
  ...
  var serviceProviderLogin = (function() {
    ...
    var init = function(modalSelector, publisherConfig, done) {
      ...
      $submit.click(function() {
        representativeName = $form.find('.representative-name').val();
        $form.submit();
      });
  ...
  var serviceProvider = (function() {
    var $el, $publisher, $subscriber, $getCustomer, $endCall,
    ...
    var getCustomer = function() {
      ...
      dequeueData.representativeName = representativeName;
    ...
    // set the session to ended
    var setToEnded = function() {
      $.ajax({
        type: 'POST',
        url: '/help/queue/' + sessionId,
        data: { '_METHOD' : 'DELETE' },
        async: false
      })
      .done(function(data) {
      ...
      .always(function() {
        console.log('set to ended request completed');
      });
      exports.onbeforeunload = undefined;
    };

    // set the session to not connected when the customer is not longer on the session
    var setToNotConnected = function() {
      $.ajax({
        type: 'POST',
        url: '/help/setToNotConnected/' + sessionId,
        data: { '_METHOD' : 'POST' },
        async: false
      })
      .done(function(data) {
      ...
      .always(function() {
        console.log('set to not connected request completed');
      });
      exports.onbeforeunload = undefined;
    };
